package starling.utils
{
    ...
    public class AssetManager
    {
		...
		private var mBitmaps:Dictionary;
        ...
        {
            if (name in mBitmaps) return mBitmaps[name]; else return null;
           
        }  
	
		 public function getCreateTexture(name:String, x:int, y:int, w:int, h:int):Texture
        {
			var bitmap:Bitmap = null;
			if (name in mBitmaps) bitmap = mBitmaps[name]; 
		    var buffer:BitmapData = bitmap.bitmapData;
		    var tempData:BitmapData = new BitmapData(w, h, true, 0x00000000);
			var rect:Rectangle = new Rectangle(x,y,w,h);
	        var tilePixels:ByteArray = new ByteArray();
			tilePixels = buffer.getPixels(rect);
			tilePixels.position = 0;
			tempData.setPixels(tempData.rect, tilePixels);
			var texture:Texture = Texture.fromBitmapData(tempData);
			return texture;
			
			
			/*
			var bitmapData:Bitmap = null;
			if (name in mBitmaps) bitmapData = mBitmaps[name]; 
			  
			var tempData:BitmapData = new BitmapData(128, 128, true, 0x00000000);
			var rect:Rectangle = new Rectangle(1,1,w,h);
	        var tilePixels:ByteArray = new ByteArray();
			tilePixels = bitmapData.bitmapData.getPixels(rect);
			tilePixels.position = 0;
			tempData.setPixels(tempData.rect, tilePixels);
			return Texture.fromBitmapData(tempData);
        ...
        // direct adding
		   ...
		   public function addXml(name:String, texture:XML):void
        {
            log("Adding xml '" + name + "'");
        ...
        public function addBitmap(name:String, texture:Bitmap):void
        {
            log("Adding bitmap '" + name + "'");
            
            if (name in mBitmaps)
                throw new Error("Duplicate bitmap name: " + name);
            else
                mBitmaps[name] = texture;
        }
