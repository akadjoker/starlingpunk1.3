package com.djoker 
{
	...
	public class Sfx 
	{
		...
		{
			if (_channel) stop();
			_pan = SP.clamp(pan, -1, 1);
			_vol = vol < 0 ? 0 : vol;
			_filteredPan = SP.clamp(_pan + getPan(_type), -1, 1);
		...
		{
			if (!_channel) return;
			value = SP.clamp(value, -1, 1);
			var filteredPan:Number = SP.clamp(value + getPan(_type), -1, 1);
		...
		*/
		static public function setPan(type:String, pan:Number):void
		{
			...
			transform.pan = SP.clamp(pan, -1, 1);
